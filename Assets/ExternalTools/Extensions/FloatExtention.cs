...
public static class FloatExtention
{
	...
	//unclamped map
	public static float UnclampedRemap (float current1, float current2, float target1, float target2, float val) {
		return FloatExtention.ULerp (target1, target2, FloatExtention.UInverseLerp (current1, current2, val));
	}
	public static void AssignRandom(this float val, float min, float max)
	{
