...
public static class ListExtensions
{
    ...
    public static List<U> GetAllDataOfType<T, U>(this List<T> list)
    {
        List<U> tempList = new List<U>();
        for (int i = 0; i < list.Count; i++)
        {
            if (list[i].GetType() == typeof(U))
            {
                tempList.Add((U)(object)list[i]);
            }
        }
    ...
    public static List<U> GetValueList<T, U>(this Dictionary<T, U> dictionary)
    {
        List<U> values = new List<U>(dictionary.Values);
        return values;
    }
    
    public static List<T> GetKeyList<T, U>(this Dictionary<T, U> dictionary)
    {
        List<T> values = new List<T>(dictionary.Keys);
        return values;
    }
}